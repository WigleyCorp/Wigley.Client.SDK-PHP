...
class IssuerData extends Sdk\Message
{
    ...
    public function __construct($data = null)
    {
        if (!array_key_exists('CommerceId', $data) || !is_numeric($data['CommerceId'])) {
            throw new \WigleyPay\Sdk\Exception\InvalidArgumentException('CommerceId debe ser un valor numérico.');
        }
        if (!array_key_exists('IssuerId', $data) || !is_numeric($data['IssuerId'])) {
            throw new \WigleyPay\Sdk\Exception\InvalidArgumentException('IssuerId debe ser un valor numérico.');
        }

...
class Certificate {
    ...
    public static function fromPfxFile($filename, $passphrase)
    {
        if (!file_exists($filename)) {
            throw new \WigleyPay\Sdk\Exception\CertificateException(sprintf('No existe el archivo \'%s\'.', $filename));
        }
        if (!is_readable($filename)) {
            throw new \WigleyPay\Sdk\Exception\CertificateException(sprintf('El archivo %s no puede ser abierto para lectura.', $filename));
        }
        if (!openssl_pkcs12_read(file_get_contents($filename), $certs, $passphrase)) {
            throw new \WigleyPay\Sdk\Exception\CertificateException(sprintf('No fue posible leer el certificado del archivo \'%s\'.', $filename));
        }
     ...
     * 
     * @param string $key
     * @return \WigleyPay\Sdk\Certificate\Certificate
     */

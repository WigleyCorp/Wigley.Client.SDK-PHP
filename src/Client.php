...
class Client implements SecurePaymentGatewayInterface
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $this->http_client = new \GuzzleHttp\Client([
            ...
            'headers' => [
                'User-Agent' => sprintf('WigleyPaySdk/%s %s', self::VERSION, \GuzzleHttp\default_user_agent()),
    ...
    /**
     *
     * @param (array|\WigleyPay\Sdk\Models\Authorization) $auth
     * @return array
     * @throws \Exception
     */
    public function Authorize($auth)
    {
        ...
        if (!($auth instanceof Models\Authorization)) {
            throw new Exception\WigleyPayException('$auth debe ser del tipo array o \WigleyPay\Sdk\Models\Authorization');
        }
    ...
    /**
     *
     * @param (array|\WigleyPay\Sdk\Models\PaymentRequest) $payment
     * @return \stdClass
     */
    public function Purchase($payment)
    {
        ...
        if (!($payment instanceof Models\PaymentRequest)) {
            throw new \Exception('$payment debe ser del tipo array o \WigleyPay\Sdk\Models\PaymentRequest');// FIXME
        }
    ...
    public function Cancel($payment)
    {
        ...
        if (!($payment instanceof Models\CancelRequest)) {
            throw new \Exception('$payment debe ser del tipo array o \WigleyPay\Sdk\Models\CancelRequest');
        }
    ...
    /**
     * @since 0.6.0
     * @return \WigleyPay\Sdk\Models\Transaction
     */
    public function Refund($payment)
    {
        ...
        if (!($payment instanceof Models\RefundRequest)) {
            throw new \Exception('$payment debe ser del tipo array o \WigleyPay\Sdk\Models\RefundRequest');
        }
    ...
    /**
     * @since 0.6.0
     * @return WigleyPay\Sdk\Models\BlacklistRequest[]
     */
    ...
    /**
     * @since 0.3.0
     * @param \WigleyPay\Sdk\Message\CreateBankInstrumentRequest $request
     * @return \WigleyPay\Sdk\PaymentInstrument
     */
     ...
     * @return \WigleyPay\Sdk\Models\Commerce
     */
    ...
    /**
     * @since 0.3.0
     * @param (array|\WigleyPay\Models\TransactionQuery) $query
     * @return \WigleyPay\Sdk\Models\TransactionCursor
     * @throws Exception\WigleyPayException
     */
    public function ObtainTransactions($query)
    {
        ...
        if (!($query instanceof Models\TransactionQuery)) {
            throw new Exception\WigleyPayException('$query debe ser del tipo array o \WigleyPay\Sdk\Models\TransactionQuery');// FIXME
        }
    ...
    /**
     * @since 0.3.0
     * @param (array|\WigleyPay\Sdk\Models\TransactionQuery) $query
     * @return string
     * @throws Exception\WigleyPayException
     */
    ...
    public function GetServerPublicKey($fingerprint)
    {
        if (!preg_match('/[0-9a-fA-F]{40}/', $fingerprint)) {
            throw new Exception\WigleyPayException('El formato de Fingerprint no es válido.');
        }
    ...
    /**
     * @since 0.3.0
     * @param (array|WigleyPay\Sdk\Models\CodeRequest) $request
     * @return WigleyPay\Sdk\Models\Transaction
     * @throws Exception\WigleyPayException
     */
    public function CodeAction($request)
    {
        ...
        if (!($request instanceof Models\CodeRequest)) {
            throw new Exception\WigleyPayException('$query debe ser del tipo array o \WigleyPay\Sdk\Models\CodeRequest');// FIXME
        }
    ...
    private function configureDefaults(array $config)
    {
        ...
        if ($base_uri = getenv('WigleyPay_ENDPOINT')) {
            $defaults['base_uri'] = $base_uri;
        }
        // @deprecated Environment variable WigleyPay_ENV is deprecated
        if ($env = getenv('WigleyPay_ENV')) {
            $defaults['env'] = $env;
        }
        if ($client = getenv('WigleyPay_CREDENTIALS_CLIENT')) {
            $defaults['client'] = $client;
        }
        if ($pem_filename = getenv('WigleyPay_CREDENTIALS_PEM_FILENAME')) {
            $defaults['pem_filename'] = $pem_filename;
        }
        if ($pfx_filename = getenv('WigleyPay_CREDENTIALS_PFX_FILENAME')) {
            $defaults['pfx_filename'] = $pfx_filename;
        }
        if ($pfx_passphrase = getenv('WigleyPay_CREDENTIALS_PFX_PASSPHRASE')) {
            $defaults['pfx_passphrase'] = $pfx_passphrase;
        }
        if ($privkey_fingerprint = getenv('WigleyPay_CREDENTIALS_PRIVKEY_FINGERPRINT')) {
    ...
    private function _exec($http_method, $path, $message = null)
    {
        ...
        if (!($certificateStore instanceof Certificate\CertificateProviderInterface)) {
            throw new Exception\WigleyPayException(sprintf('%s debe implementar la interfaz WigleyPay\Sdk\Certificate\CertificateProviderInterface.', get_class($certificateStore)));
        }
        if (preg_match('/^Key\/([A-Z0-9]{40})$/', $path, $matches)) {
            if ($response_obj['Object']['Object']['Response']['Fingerprint'] === $matches[1]) {
                if ($matches[1] ==! $response_obj['Object']['Object']['Response']['Fingerprint']) {
                    throw new Exception\WigleyPayException('No fue posible obtener el certificado del servidor.');
                }
